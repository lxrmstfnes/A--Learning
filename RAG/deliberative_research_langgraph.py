...
# 设置API密钥（兼容 DASHSCOPE_API_KEY 与 OPENAI_API_KEY）
DASHSCOPE_API_KEY = os.getenv("DASHSCOPE_API_KEY") or os.getenv("OPENAI_API_KEY")

if not DASHSCOPE_API_KEY:
    raise ValueError(
        "未找到 DashScope API 密钥。请先设置环境变量 DASHSCOPE_API_KEY 或 OPENAI_API_KEY：\n"
        "  1. 在 https://dashscope.console.aliyun.com/ 获取 API Key\n"
        "  2. 在终端执行: export DASHSCOPE_API_KEY='你的密钥'\n"
        "  3. 或写入 ~/.zshrc 后执行: source ~/.zshrc"
    )
